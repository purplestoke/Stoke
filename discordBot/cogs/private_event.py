...
class private_event(commands.Cog):
    ...
    async def setup(self, interaction: Interaction, event):
        ...
        logFlags = []
        logIns = stokeIns.Log(
            bugsId=None, guildId=guildId, flags=logFlags, discord=discord
        )
        ...
        try:
            ...
            flag = 3
            logFlags.append(flag)
            logIns.privEventLog(_cmd="setup")
    ...
    async def finsished(self, interaction: Interaction):
        ...
        logFlags = []
        logIns = stokeIns.Log(
            bugs_id=None, guildId=guildId, flags=logFlags, discord=discord
        )
        ...
        try:
            ...
            flag = 3
            logFlags.append(flag)
            logIns.privEventLog(_cmd="teardown")
            await interaction.response.send_message("e")
            return
    ...
    async def draw(self, interaction: Interaction, event):
        ...
        privEventIns = stokeIns.PrivateEvent(event)
        # CHECK IF USER HAS ADMIN ROLE
        if discord == purpleStoke:
            drawFlag, winString = privEventIns.privEventDraw()
            logFlags.append(drawFlag)
            if drawFlag == 2:
                logIns.privEventLog(_cmd="draw")
    ...
    async def compete(self, interaction: Interaction, event):
        # VARIABLES
        mem = interaction.user
        discord = mem.id
        guild = interaction.guild
        ...
        guildIns = stokeIns.Guilds(find="privEvent", guildId=guildId)

        # CHECK IF IN COMMUNITY EVENT CHANNEL
        f, privateChannel = guildIns.pullChannel()
        if interaction.channel.id == privateChannel:
            pass
        else:
            logIns.privEventLog(_cmd="compete")
            await interaction.response.send_message(f"e {f}")
            return

        # CHECK IF USER CAN COMPETE IN EVENT
        try:
            bugsIdFlag, bugsId = bugsProfileIns.bugsId()
            ...
            if bugsIdFlag == 2:
                ...
                return
            privEventIns = stokeIns.PrivateEvent(event)
            compListFlag, retCompetitors = privEventIns.competitorList()
            ...
            for id in retCompetitors:
                if id == bugsId:
                    ...
                    roleFlag, role = guildIns.pullRole(_find="privEventRole")
    ...
    async def on_message(self, message):
        ...
        logFlags = []
        logIns = stokeIns.Log(
            bugs_id=None, guildId=guildId, flags=logFlags, discord=str_sender
        )
        ...
        try:
            ...
            logFlags.append(3)
            logIns.privEventLog(_cmd="scSubmission")
            return
        except:
            return
